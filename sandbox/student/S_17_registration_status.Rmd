...
s_17_registration_status_test_df <- tibble(
  ...
  graduation_date_year = sample(1978:2010, 13, replace = TRUE),
  graduation_date_month = sample(01:12, 13, replace = TRUE),
  graduation_date_day = sample(01:29, 13, replace = TRUE)) %>% 
  ...
  mutate(graduation_date = paste0(graduation_date_year, graduation_date_month, graduation_date_day))
 
 s_17_registration_status_test_df <- s_17_registration_status_test_df %>% 
  mutate(graduation_date = as_date(graduation_date)) %>% 
   select(student_type_code , graduation_date)
...
change_s_17_registration_status_df <- s_17_registration_status_test_df %>%
   mutate(s_reg_status = case_when(
      ...
      student_type_code == "F" ~ 'FF', # TODO  New Freshmen -- Freshman start date > than 1 year since high school grad date
      student_type_code == "F" ~ 'FH', # TODO New Freshman from HS -- based on grad date within 1 year of high school grad date
