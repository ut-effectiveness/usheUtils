...
fake_building_df <- data.frame(
  location_code = sample(c("", NA), sample_size, replace = TRUE),
  ownership_code = sample(c("", NA), sample_size, replace = TRUE),
  ...
  building_cost_replacement = sample(c("", NA), sample_size, replace = TRUE),
