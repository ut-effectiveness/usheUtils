...
real_df <- get_data_from_sql_file(file_name="c_50.sql",
                             dsn="edify")
...
change_c_50_delivery_model_df <- c_50_delivery_model_test_df %>%
   mutate(c_50_delivery_model = case_when(
    ...
    instruction_method_code == "V" ~ '?',
    instruction_method_code == " " ~ "P",
    instruction_method_code == "E" ~ 'H',
    instruction_method_code == "R" ~ '?',
    instruction_method_code == "I" ~ '?',
    instruction_method_code == "P" ~ '?',
    instruction_method_code == "H" ~ '?',
...
```{r}
DT::datatable(format_place_holder(c_50_delivery_model_test_df), rownames=FALSE)
```

c_50_delivery_model_real_df for the real data
```{r}
DT::datatable(format_place_holder(c_50_delivery_model_real_df), rownames=FALSE)
```
